...
exports.handler = async (event, context) => {
  try {
    ...
    console.log('THIS IS event.querySTringParameters.name', event);
    const response = await fetch(`https://pokedex-alchemy.herokuapp.com/api/pokedex?pokemon=${event.queryStringParameters.name}`);
    const json = await response.json();
    console.log('HELLOOOOO', json);
    return { 
      statusCode: 200, 
